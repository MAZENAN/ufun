...
function to_under($name) {
    ...
    return preg_replace('@^_@', '', preg_replace_callback('@[A-Z]@', create_function('$val', 'return \'_\'.strtolower($val[0]);'), $name));
}
...
function to_camel($name) {
    ...
    return ucfirst(preg_replace_callback('@_[A-Za-z]@', create_function('$val', 'return strtoupper(substr($val[0],1,1));'), $name));
}

if (!function_exists('array_replace')) {

    /**
     * array_replace 函数缺少使用
     * @param string $name
     * @return string
     */
...
function _simple_preg($rule) {
    ...
    $ret = preg_replace_callback('@\\\{([a-zA-Z0-9_-]*):([^}]+)\\\}@', '_replace_keys', $rule);
    return '@^' . $ret . '@';
}

function _route_url_replace($url, &$args) {
    return preg_replace('@\{([a-zA-Z0-9_-]*):([^}]+)\}@e', '(\'$1\'!=\'\'&&isset($args[\'$1\']))?$args[\'$1\']:\'$1\'', $url);
}
