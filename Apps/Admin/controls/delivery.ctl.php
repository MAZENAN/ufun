...
class DeliveryController extends SmcmsController{
    ...
    public function addPostAction() {
        ...
        $row = DB::getone('SELECT id FROM @pf_delivery WHERE merchant_id=? AND week=?',[$merchantId,$week]);
        if ($row){
            $this->error('添加失败,已存在该时间点','/admin/delivery/index?merchant_id='.$merchantId);
        }
        $model = Model($this->ModelName);
        $this->setModel($model);
        $model->autoComplete();
