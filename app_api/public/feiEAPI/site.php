<?php
    ...
    public function feieyunPrint($orderInfo) {



        //经过各种条件查询数据库得到已绑定的打印机编号和key。。。。。
        $arr['sn'] = '920503519';
        $arr['key'] = 'q6m5y7pd';
        ...
        //调用下一个函数
        //********************************************
        $restatus = $this->sendFreeMessage($arr);
        logger($restatus,'feiEYun.log');
//        logger($orderInfo,'feiEYun.log');
        //********************************************
    }
    ...
    //调用飞鹅云接口代码
    //********************************************
    public function sendFreeMessage($arr)
    {
        $content = array(
            'sn'=>$arr['sn'],  
            'printContent'=>$arr['orderInfo'],
            'key'=>$arr['key'],
            'times'=>'1'//打印次数
        );
