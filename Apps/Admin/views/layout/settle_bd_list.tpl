...
<td align="center">
	<input type="checkbox" name="id[]" class="checkbox" value="{@$srs.id@}"/>
</td>

<td align="center">
	<span class="blu">{@$srs.seller_id|smval:'@pf_member':'nickname'@}</span>
</td>
<td align="center">
...
<td align="center">
	<span >{@$srs.orderid@}</span>
</td>
<td align="center">
	{@$srs.paid@}&nbsp;元
</td>
<td align="center">
	{@if $srs.scommision !=''@}{@$srs.scommision@}&nbsp;元{@/if@}
</td>
<td align="center">
...
<td align="center">
	...
	<a class="samao-link-minibtn" href="__SELF__/updatePrice?id={@$srs.id@}{@if $key@}&key={@$key@}{@/if@}">修改结算价格</a>
</td>
</tr>
{@/foreach@}
<tr>
    {@if $key == ''@}
	{@$price=DB::getone("select SUM(scommision) from @pf_order as o join @pf_camp join @pf_member as mb on o.campid=@pf_camp.id and  @pf_camp.seller_id = mb.id where state in(?,?) and refund = ? and crm_state in(?,?) and settle_state = ? and @pf_camp.seller_id=?" ,[3,8,0,2,10,0,$srs.seller_id])@}
	{@$refund=DB::getone("select SUM(scommision) from @pf_order as o join @pf_camp join @pf_member as mb on o.campid=@pf_camp.id and  @pf_camp.seller_id = mb.id where state =? and refund = ? and crm_state =? and settle_state = ? and @pf_camp.seller_id=?" ,[5,2,2,4,$srs.seller_id])@}
    {@elseif $key == 4@}
        {@$price=DB::getone("select SUM(scommision) from @pf_order as o join @pf_camp join @pf_member as mb on o.campid=@pf_camp.id and  @pf_camp.seller_id = mb.id where state in(?,?) and refund = ? and crm_state in(?,?) and settle_state = ? and @pf_camp.seller_id=?" ,[3,8,0,2,10,1,$srs.seller_id])@}
	{@$refund=DB::getone("select SUM(scommision) from @pf_order as o join @pf_camp join @pf_member as mb on o.campid=@pf_camp.id and  @pf_camp.seller_id = mb.id where state =? and refund = ? and crm_state =? and settle_state = ? and @pf_camp.seller_id=?" ,[5,2,2,5,$srs.seller_id])@}
    {@/if@}
        <td colspan="4" align="center" style="font-weight: bold;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;供应商名称：<span>{@$ttl@}</span>
