...
class Wxpay {
    public function send($v_oid, $v_amount, $subject, &$error) {
...
//            $input->SetTotal_fee(intval($v_amount*100));
            ...
            $input->SetNotify_url("http://" . $_SERVER['HTTP_HOST'] . "/app_api/c/public/wxpayApp_notify.php");
    ...
    function wxRefund($order)
    {
        ...
        $input->SetOut_trade_no($order['orderid']);            //自己的订单号
//        $input->SetTransaction_id($order['transaction_id']);    //微信官方生成的订单流水号，在支付成功中有返回
        $input->SetOut_refund_no($order['id']);            //订单编号
        $input->SetTotal_fee($order['need_topay']*100);            //订单标价金额，单位为分
        $input->SetRefund_fee($order['paid']*100);            //退款总金额，订单总金额，单位为分，只能为整数
