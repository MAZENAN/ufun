...
class Pagelist {
    ...
    public function getinfo() {
        ...
        if ($this->records_count == -1) {
           // var_dump(strripos($this->sql, ' from '));
           // var_dump(stripos($this->sql, ' from '));

            if (strripos($this->sql, ' from ') === stripos($this->sql, ' from ')) {
                $sql = preg_replace('@^select\s+(distinct\s+[a-z][a-z0-9]+\s*,)?(.*)\s+from\s+@', 'select $1count(1) as MyCount from ', $this->sql);
                $row = DB::getone($sql, $this->args);
            } else {//var_dump( $this->sql,$this->args);die();
                $row = DB::getone('select count(1) as MyCount from (' . $this->sql . ') MyTempTable', $this->args);
                
            }
            $this->records_count = $row['MyCount'];
        }
    ...
    public function getlist($fetch_style = PDO::FETCH_ASSOC) {
        ...
        return DB::getlist($sql, $this->args, $fetch_style);
    }

    public function getall($fetch_style = PDO::FETCH_ASSOC) {
        return DB::getlist($this->sql, $this->args, $fetch_style);
    }
